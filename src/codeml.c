...
enum {
   ...
   FromCodon0 = 5, FromCodon = 6, REVaa_0 = 8, REVaa = 9
} AAModel;

char *aamodels[] = { "Poisson", "EqualInput", "Empirical", "Empirical_F", "",
     "FromCodon0", "FromCodon", "", "REVaa_0", "REVaa" };
enum {
...
int main(int argc, char *argv[])
{
   ...
   */
   ...
   else if (com.seqtype == CODONseq || com.model == FromCodon0 || com.model == FromCodon)
      nc = 64;
   ...
   for (idata = 0; idata < com.ndata; idata++) {
      ...
      fprintf(fout, "Model: %s, ", pmodel);
      if (com.clock) fprintf(fout, " %s ", clockstr[com.clock]);
      if (com.seqtype == CODONseq || com.model == FromCodon0 || com.model == FromCodon) {
      ...
      if ((com.space = (double*)realloc(com.space, com.sspace)) == NULL) {
         printf("\nfailed to get %9zu bytes for space", com.sspace);
         error2("oom space");
      }

      if (com.seqtype == AAseq) {
         InitializeBaseAA(fout);
         if (com.model == FromCodon0 || com.model == FromCodon /* ||com.aaDist==AAClasses */) {
            AA2Codonf(com.pi, com.fb61);  /* get codon freqs from aa freqs */
            if (com.model == FromCodon0) {
               xtoy(com.fb61, com.pi, Nsensecodon);
            }
         }
      }
      ...
      if ((com.runmode == -2 || com.runmode == -3) && com.Mgene != 1) {
         ...
         if (com.seqtype == AAseq && com.model == FromCodon0) {
            ...
            for (i = 0; i < com.ns; i++) for (j = 0; j < com.npatt; j++)
               com.z[i][j] += 64;
            for (j = 64; j < 64 + 20; j++)
               getAAstr(CODONs[j], j - 64);
            CODONs[j][0] = CODONs[j][1] = CODONs[j][2] = '*';
            for (i = 0; i < com.ncode; i++)
               getcodon(CODONs[i], FROM61[i]);
         }
         ...
         printf("\n%9zu bytes for distance", com.ns*(com.ns - 1) / 2 * sizeof(double));
         printf("\n%9zu bytes for conP\n", com.sconP);
...
int Forestry(FILE *fout)
{
   ...
   if (com.seqtype == 1 && com.aaDist >= FIT1) {
      ...
      zero(paa0, 20);
      for (i = 0; i < com.ncode; i++)
         paa0[GeneticCode[com.icode][FROM61[i]]] += pcodon0[i];
   ...
   for (itree = 0; ntree == -1 || itree < ntree; itree++, iteration = 1) {
      ...
      if (com.sspace < spaceming2(np)) {
         ...
         printf("\nspace adjusted to %9zu bytes\n", com.sspace);
         if ((com.space = (double*)realloc(com.space, com.sspace)) == NULL) {
            printf("\ntrying to get %zu bytes for ming2", com.sspace);
            error2("oom space");
         }
      }
      ...
      */
      ...
      fprintf(frst1,"\t%.3f", -lnL);
      */
      fprintf(fout, "\n");  OutTreeN(fout, 0, 1);   fprintf(fout, "\n");
...
int sortwM3(double x[])
{
   ...
   */
   ...
   for (i = 0; i < com.ncatG; i++) com.rK[i] = space[index[i]];
   xtoy(com.freqK, space, com.ncatG);
   for (i = 0; i < com.ncatG; i++) com.freqK[i] = space[index[i]];
   for (i = 0; i < com.ncatG-1; i++) x[k + i] = com.freqK[i];
   for (i = 0; i < com.ncatG; i++)   x[k + com.ncatG - 1 + i] = com.rK[i];
   return(0);
}
...
void DetailOutput(FILE *fout, double x[], double var[])
{
   ...
   */
   ...
   double t, *tdSdNb = NULL, y; /* 3normal: mu0=0 fixed. mu2 estimated */
   double fb3x4[12];
   ...
   if (com.seqtype == CODONseq || com.model == FromCodon0 || com.model == FromCodon) {
      if (com.hkyREV) {
         ...
         for (i = 0; i < 5; i++) fprintf(fout, "%8.5f ", x[k++]);
         fprintf(fout, "\n");
      }
      ...
      */
      if (com.NSsites && (com.model == 0 || com.model == FromCodon0)) {
      ...
      if (!com.fix_omega && com.NSsites == 0 && (com.model == 0 || com.model == FromCodon0) && com.aaDist != 7 && com.Mgene <= 2)
         fprintf(fout, "\nomega (dN/dS) = %8.5f\n", x[k++]);
      ...
      if (com.NSsites == NSdiscrete && com.aaDist) { /* structural site classes */
         ...
         for (j = 0, k += com.ncatG - 1; j < com.ncatG; j++) {
            fprintf(fout, "%d: f=%8.5f, ", j + 1, com.freqK[j]);
            for (i = 0; i < npclass; i++) fprintf(fout, "%9.5f", x[k++]);
         ...
         k = com.ntime + com.nrgene + com.nkappa + com.npi;
         for (i = 0; i < 4 + (com.aaDist == FIT2); i++)
            fprintf(fout, " %9.5f", x[k++]);  
         fprintf(fout, "\n");
      }
      else if ((com.model == 0 || com.model == FromCodon0) && com.NSsites == 0 && !com.fix_omega && com.Mgene > 2) {
   ...
   if (com.aaDist == AAClasses) {
      if (com.model == 0 || com.model == FromCodon0) {
   ...
   if (com.seqtype == CODONseq && com.ngene == 1 && (com.model == 0 || com.model == FromCodon0 || com.NSsites == 0)) {
      ...
      for (i = 0, dNt = dSt = 0; i < tree.nbranch; i++) {
         ...
         if (com.NSsites == 0) {
            ...
            else if (com.model == 0 || com.model == FromCodon0 || com.model == FromCodon)
            ...
            */
            N = com.ls * 3 - S;
            if (com.model >= NSbranchB &&com.model <= NSbranch3) {
               tdSdNb[i] = t;
      ...
      if (com.model >= NSbranchB &&com.model <= NSbranch3 && com.NSsites == 0) {
         fprintf(fout, "\ndS tree:\n");
...
int getnrate(int firsttime)
{
   ...
   if (com.seqtype == AAseq || com.seqtype == CODON2AAseq) {
      ...
      if (com.hkyREV && (com.model == FromCodon0 || com.model == FromCodon))  /*  REV & FromCodon not well-tested. */
      ...
      if (com.Mgene > 2 || (com.Mgene == 2 && (com.model == Fequal || com.model == 2)))
         error2("Mgene && model");
      if (com.seqtype == 2 && com.model != FromCodon0 && com.model != FromCodon && com.model != AAClasses) {
      ...
      if ((com.model && com.model != FromCodon0 && com.aaDist)
         && ((com.model > NSbranch2 && com.model != FromCodon0) || com.aaDist != AAClasses))
         error2("model & aaDist");
      ...
      if (com.aaDist != AAClasses) {
         ...
         if (com.model > NSbranch3 && com.model != FromCodon0)
            error2("seqtype or model.");
...
int GetOptions(char *ctlf)
{
   ...
   char *optstr[] = { "seqfile", "outfile", "treefile", "seqtype", "noisy",
        "cleandata", "runmode", "method", "clock", "TipDate",
        "getSE", "RateAncestor", "CodonFreq", "estFreq", "verbose",
        "model", "hkyREV", "aaDist","aaRatefile", "NSsites",
        "NShmm", "icode", "Mgene", "fix_kappa", "kappa",
        "fix_omega", "omega", "fix_alpha", "alpha", "Malpha",
        "ncatG", "fix_rho", "rho", "ndata", "bootstrap", "Small_Diff", "fix_blength" };
   double t;
   FILE  *fctl;
         ...
         if (strncmp(opt, optstr[iopt], 8) == 0) {
            ...
            case (21):
               ...
               if (com.seqtype == 1 && (com.clock == 5 || com.clock == 6)) {
                  ...
                  for (j = 0; j < min2(ng, com.ndata); j++) printf("%4d", data.icode[j]);
                  printf("\n");
               }
               break;
   ...
   if (noisy) printf("\n");
   if (com.seqtype == 1 || com.model == FromCodon0 || com.model == FromCodon)
      setmark_61_64();
   ...
   if (com.method && com.seqtype == 2 && (com.model == FromCodon0 || com.model == FromCodon)) {
   ...
   if (com.aaDist <= 6 && (com.seqtype == CODONseq || com.model == FromCodon0 || com.model == FromCodon))
      strcpy(com.daafile, daafiles[abs(com.aaDist)]);

   if (com.fix_alpha && com.alpha == 0) {
      if (com.rho) puts("rho set to 0.");
      com.fix_rho = 1;  com.rho = 0;
   }
...
int SetxBound(int np, double xb[][2])
{
   ...
   double rgeneb[] = { 0.01,99 }, rateb[] = { 1e-4,999 };
   ...
   else if ((com.seqtype == CODONseq || com.model == FromCodon0 || com.model == FromCodon) && com.aaDist != AAClasses)
   {
   ...
   if (com.aaDist < 0 && (com.seqtype == 1 || com.model == FromCodon0 || com.model == FromCodon)) {
   ...
   if (com.seqtype == CODONseq)
      ...
      if (com.aaDist == AAClasses) {
         ...
         if (com.seqtype == AAseq) {   /* AAseq */
            ...
            else if (com.model == FromCodon0 || com.model == FromCodon) {
...
int GetCodonFreqs(void)
{
   ...
   */
   int n = com.ncode, i, j, ic, iaa, b[3];
...
int SetParameters(double x[])
{
   ...
   */
   ...
   if (com.seqtype == 1 || com.model == FromCodon0 || com.model == FromCodon || com.aaDist == AAClasses) {
      k = com.ntime + com.nrgene;
      if (com.hkyREV == 0) {
   ...
   if (com.nrate) {
      ...
      else if ((com.model == 0 || com.model == FromCodon0) && com.NSsites == 0 && com.Mgene <= 1)
   ...
   */
   ...
   if (com.NSsites >= NSgamma) {
      ...
      if (status) {
         ...
         for (j = 0; j < (com.NSsites == 7 ? 2 : 4); j++) printf(" %12.6f", par[j]);
         printf("\n");
         for (j = 0; j < com.ncatG; j++) printf("%13.5f", com.freqK[j]);
         printf("\n");
         for (j = 0; j < com.ncatG; j++) printf("%13.5e", com.rK[j]);
         printf("\n");
      }
   }
   return(0);
   ...
   double Root[], double U[], double V[], double *meanrate, double kappa[], double omega, double Q[])
{
   ...
   */
   ...
   if (com.seqtype == AAseq && com.model == FromCodon) return (0);  /* the FromCodon model */
...
void CountCodons(FILE *fout, double fcodonsg[], double fb3x4sg[], double fb4g[])
{
   ...
   */
   int h, j, k, nc = NCODE, ig, wname = 15, nb[3], ib[3][4], ic, status = 0;
   ...
   double fb40[], double fb4[])
{
   ...
   */
   ...
   for (h = com.posG[ig]; h < com.posG[ig + 1]; h++) {
      ...
      for (i0 = k = 0, t = 0; i0 < nb[0]; i0++) {
         for (i1 = 0; i1 < nb[1]; i1++) for (i2 = 0; i2 < nb[2]; i2++) {
   ...
   */
   ...
   if (com.verbose && com.ngene == 1) {
      ...
      for (k = 0; k < 64; k++) {
         ...
         if ((k + 1) % 4 == 0) fprintf(fout, "\n");
      }
      /*
      int wrongorder[4] = { 2,1,3,0 };
...
int GetDaa(FILE* fout, double daa[])
{
   ...
   */
   ...
   SetAA1STEP();
   frpintf(frst, "\n");
   for(i=0; i<naa;i++, frpintf(frst, "\n"))
      for(j=0; j<i; j++)
         fprintf(frst,"%3d",AA1STEP[i*(i-1)/2+j]);
...
int GetCodonFreqs2(void)
{
   ...
   */
   ...
   double *pi = com.pi, fb3x4[12], fb4[4];
...
int distance3pos(double dHKY[], double kHKY[], int *sites4, char *z1, char *z2)
{
   ...
   *sites4 = (int)sum(fij[3], 16);
   ...
   if (*sites4)
      for (k = 0; k < 16; k++) fij[3][k] /= *sites4;
   for (i = 0; i < 4; i++) for (j = 0; j < 4; j++) pi4[i] += fij[3][i * 4 + j] / 2;
   for (i = 0; i < 4; i++) for (j = 0; j < 4; j++) pi4[j] += fij[3][i * 4 + j] / 2;
...
int PairwiseCodon(FILE *fout, FILE*fds, FILE*fdn, FILE*ft, double space[])
{
   ...
   */
   ...
   for (k = 0; k < npatt0; k++) fpatt0[k] = (float)com.fpatt[k];
      ...
      for (js = 0; js < is; js++) {
         ...
         if (com.cleandata) {
            ...
            for (h = 0, com.ls = 0; h < npatt0; h++) {
               for (i0 = 0; i0 < 2; i0++)
                  for (k = 0; k < 3; k++)
                     codon[i0][k] = pz0[i0 == 0 ? is : js][h * 3 + k];
         ...
         if (noisy >= 9) {
            printf("\n");
            for (k = 0; k < np; k++) printf(" %12.6f", x[k]);
            printf("\n");
            for (k = 0; k < np; k++) printf(" %12.6f", xb[k][0]);
            printf("\n");
            for (k = 0; k < np; k++) printf(" %12.6f", xb[k][1]);
            printf("\n");
         }
         ...
         lnLmodel = lnL;
         fprintf(fout, "\nlnL =%12.6f\n", -lnL);
         for (k = 0; k < np; k++) fprintf(fout, " %8.5f", x[k]);
         fprintf(fout, "\n");
         ...
         if (noisy >= 3) {
            ...
            printf("\ndHKY (123-4):");  for (k = 0; k < 4; k++) printf(" %8.5f", dHKY[k]);
            printf("\nkHKY (123-4):");  for (k = 0; k < 4; k++) printf(" %8.5f", kHKY[k]);
...
double lfun2AArev(double x[], int np)
{
   ...
   */
   int i, j, k, n = 20;
   double pi[3][20], *p, Q[3][400], *Fe = Q[0], t, t1, t2, m1, m2, lnL = 0;
...
int PairwiseAA(FILE *fout, FILE*f2AA)
{
   ...
   */
   ...
   for (j = 0; j < com.ns; j++)  pz0[j] = com.z[j];
   ...
   for (j = 0; j < com.ns; j++)  com.z[j] = pz0[j];
   return (0);
}
   ...
   */
   ...
   if (c < n) {
      ...
      for (k = 0, aa = -1; k < nChara[c]; k++) {
         newaa = GeneticCode[com.icode][FROM61[(int)CharaMap[c][k]]];
         if (newaa == -1) continue;
         newaa = AAs[newaa];
         if (aa == -1) {
            naa = 1;
...
int GetCategoryQc(char z[NS])
{
   ...
   */
   ...
   for (i = 0; i < 3; i++) {
      for (j = 0; j < 4; j++)  markb[j] = 0;
      for (j = 0; j < com.ns; j++) markb[b[j][i]] = 1;
...
void SimulateData2s61(void)
{
   ...
   */
   ...
   for (ip = 0; ip < 99; ip++) {
      ...
      printf("\nw: "); for (i = 0; i < com.ncatG; i++) printf("%7.4f", com.rK[i]); 
      printf("\n");
      fprintf(frst, "\nSite classe model (K=%d)\np: ", com.ncatG);
      for (i = 0; i < com.ncatG; i++)
         fprintf(frst, "%7.4f", com.freqK[i]);
      fputs("\nw: ", frst); for (i = 0; i < com.ncatG; i++) fprintf(frst, "%7.4f", com.rK[i]); 
      fprintf(frst, "\n"); 
      ...
      for (il = 0; il < nil + 1; il++) {
         ...
         if (nr > 1) {
            for (j = 0; j < 5; j++) vx[j] = sqrt(vx[j] / (nr - 1.) / nr);
            mse[0] = sqrt(mse[0] / nr); 
         }
         fprintf(frst, "%4d ", com.ls);
         for (j = 0; j < 5; j++) fprintf(frst, "%7.4f +%7.4f", mx[i], vx[i]);  
         fprintf(frst, "\n"); 
      }  /* for (ii) */
   }   /* for(ip) */
   exit(0);
   ...
   */
   ...
   for (ip = 0; ip < 99; ip++) {
      ...
      for (il = 0; il < nil; il++) {
         ...
         fprintf(frst, "%4d ", com.ls);
         for (j = 0; j < 5; j++)  fprintf(frst, "%7.3f +%7.4f", mx[i], vx[i]);
         fprintf(frst, "\n");
         fflush(frst);
...
int mergeSeqs(FILE*fout)
{
   ...
   */
   ...
   noisy = 0;
   for(i=0; i<ns0; i++) 
      if ((z0[i] = (char*)malloc(lswhole * sizeof(char))) == NULL)
         error2("oom z");
   for (ifile = 0, ls0 = 0; ifile < nfile; ifile++) {
      ...
      for (i = 0; i < ns0; i++) if (strcmp(spname0[i], com.spname[i])) error2("spname different");
      for (i = 0; i < ns0; i++)  for (h = 0; h < com.ls; h++)
         z0[i][ls0 + h] = com.z[i][h];
      ls0 += com.ls;
      printf(" + %5d = %5d\n", com.ls, ls0);
   }
   fprintf(fout, "%6d %6d  G\nG %4d ", ns0, ls0, nfile);
   for (ifile = 0; ifile < nfile; ifile++)
      fprintf(fout, " %4d", lgene0[ifile]);
   fprintf(fout, "\n");
   ...
   */
   ...
   for (ii = 0; ii < nii; ii++) {
      ...
      for (i = 0, zero(fij4f, 16); i < n; i++) {
         ...
         ic1 -= b[0][2];
         for (k = 0; k < 4; k++)  if (GeneticCode[com.icode][ic1 + k] != iaa)  break;
         if (k < 4) continue;
         for (j = 0; j < n; j++) {
      ...
      for (k = 0; k < 4; k++) pi4f[k] = sum(fij4f + k * 4, 4);
...
double distanceHKY85(double x[], double *kappa, double alpha)
{
   ...
   *kappa = 0;
   for (i = 0, zero(p, 4); i < 4; i++) {
      for (j = 0; j < 4; j++) { p[i] += x[i * 4 + j] / 2;  p[j] += x[i * 4 + j] / 2; }
   }
   P1 = x[0 * 4 + 1] + x[1 * 4 + 0];
   P2 = x[2 * 4 + 3] + x[3 * 4 + 2];
...
int lfunNSsites_M2M8(FILE* frst, double x[], int np)
{
   ...
   */
   ...
   if (com.NSsites == 8 && trianglePriorM8) {
      ...
      for (j = 0; j < n1d; j++) printf("%9.4f", (2 * j + 1.) / (2 * n1d));
      printf("\n");
      for (j = 0; j < n1d; j++) printf("%9.4f", lnprior[j]);
      printf("\n");
   }
